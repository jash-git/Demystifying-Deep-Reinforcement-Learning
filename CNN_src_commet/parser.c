...
list *read_cfg(char *filename)
{
    ...
    if(file == 0) file_error(filename);//文件为空返回错误
    char *line;
    int nu = 0;
    list *sections = make_list();//list 里面是一个前后节点node 还有size
    section *current = 0; //section里面是list 另外还有一个char＊的 type
    ...
    while((line=fgetl(file)) != 0){
        ...
        switch(line[0]){
            case '[':
                ...
                list_insert(sections, current);// 将当前的section插入到 sections这个list当中
