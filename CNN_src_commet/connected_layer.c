...
connected_layer make_connected_layer(int batch, int inputs, int outputs, ACTIVATION activation, int batch_normalize)
{
    ...
    l.weights = calloc(outputs*inputs, sizeof(float));//权重的个数，相当于说最后有outputs 个输出；每一个输出对于每一个样本点都有一个weight，也就是inputs个；所以总要outputs*inputs个
    l.biases = calloc(outputs, sizeof(float));// 偏置只需要 outputs个。


    //float scale = 1./sqrt(inputs);
    float scale = sqrt(2./inputs);
    for(i = 0; i < outputs*inputs; ++i){
        l.weights[i] = scale*rand_uniform(-1, 1);//在 scale*（－1，1）之间产生一个随机数
    }
...
{
    axpy_cpu(l.outputs, learning_rate/batch, l.bias_updates, 1, l.biases, 1);//通过 l.bias_updates和 learning_rate/batch 来更新每一个偏置，放入到l.biases当中
    scal_cpu(l.outputs, momentum, l.bias_updates, 1);//将l.bias_updates的每一个值进行 momentum的尺度变换

    if(l.batch_normalize){
        axpy_cpu(l.outputs, learning_rate/batch, l.scale_updates, 1, l.scales, 1);// update l.scales
...
{
    int i;
    fill_cpu(l.outputs*l.batch, 0, l.output, 1);// initalize l.putput with 0
    int m = l.batch;
    int k = l.inputs;
    int n = l.outputs;
    float *a = state.input;//input data
    float *b = l.weights;
    float *c = l.output;

    //对m个样本当中的每一个样本的输入数据，使用l.weights当中的每一个权重和对应的数据点进行乘积
    gemm(0,1,m,n,k,1,a,k,b,k,1,c,n);
    if(l.batch_normalize){
        if(state.train){
            mean_cpu(l.output, l.batch, l.outputs, 1, l.mean);// 对每一个样本的输出l.output（其实可以看做是一张 feature map）.
            variance_cpu(l.output, l.mean, l.batch, l.outputs, 1, l.variance);

            scal_cpu(l.outputs, .95, l.rolling_mean, 1);
            axpy_cpu(l.outputs, .05, l.mean, 1, l.rolling_mean, 1);// 将原来的l.rolling_mean 变为0.95倍，然后加上mean的 0.05倍 构成现在的l.rolling_mean
        ...
        scale_bias(l.output, l.scales, l.batch, l.outputs, 1);// 对输出再做一个尺度变换
    }
    for(i = 0; i < l.batch; ++i){
        axpy_cpu(l.outputs, 1, l.biases, 1, l.output + i*l.outputs, 1);// 对每一个再加上偏置
    }
    activate_array(l.output, l.outputs*l.batch, l.activation);//对每一个点进行激活
}
...
void backward_connected_layer(connected_layer l, network_state state)
{
    ...
    for(i = 0; i < l.batch; ++i){
        axpy_cpu(l.outputs, 1, l.delta + i*l.outputs, 1, l.bias_updates, 1);// l.bias_updates是输出，通过每一个样本的l.delta 来更新输出的偏置
    }
    if(l.batch_normalize){
