...
// 首先计算出经过它的 feature map的尺度 l.h是输入的height
...
// 其实和卷积没什么差别，只是输出的 feature map变大了 有点像上采样了
deconvolutional_layer make_deconvolutional_layer(int batch, int h, int w, int c, int n, int size, int stride, ACTIVATION activation)
{
    ...
    int out_h = deconvolutional_out_height(l);// 就这里和卷积不一样
...
{
    int i;
    int out_h = deconvolutional_out_height(l);// 首先计算生成的feature map的大小，conv的逆过程
    int out_w = deconvolutional_out_width(l);
    int size = out_h*out_w;// 输出的特征图的大小

    int m = l.size*l.size*l.n;// 卷积核的
    int n = l.h*l.w;// 输入数据的大小
    int k = l.c;// 输入数据的维度

    fill_cpu(l.outputs*l.batch, 0, l.output, 1);// 用0进行output初始化，里面的1表示挨个index过去给output初始化

    for(i = 0; i < l.batch; ++i){
        float *a = l.filters;// 这和卷积当中不一样，卷积中是定义在循环外面的
        float *b = state.input + i*l.c*l.h*l.w;// 输入的数据在这里
        float *c = l.col_image;
...
void backward_deconvolutional_layer(deconvolutional_layer l, network_state state)
{
    ...
    for(i = 0; i < l.batch; ++i){
        ...
        float *a = state.input + i*m*n;// 这和卷积当中不一样，卷积中是定义在循环外面的
        float *b = l.col_image;
