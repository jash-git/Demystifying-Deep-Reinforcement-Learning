...
cost_layer make_cost_layer(int batch, int inputs, COST_TYPE cost_type, float scale)
{
    ...
    l.scale = scale;
    l.batch = batch;
    l.inputs = inputs;// input size
    l.outputs = inputs;// output size = input size
    l.cost_type = cost_type;
    l.delta = calloc(inputs*batch, sizeof(float));
    l.output = calloc(inputs*batch, sizeof(float));
    l.cost = calloc(1, sizeof(float));// cost is just a  value?
    #ifdef GPU
...
void forward_cost_layer(cost_layer l, network_state state)
{
    ...
    if(l.cost_type == MASKED){
        ...
        for(i = 0; i < l.batch*l.inputs; ++i){
            if(state.truth[i] == SECRET_NUM) state.input[i] = SECRET_NUM;//SECRET_NUM=-1234
        }
    }
    if(l.cost_type == SMOOTH){
         //通过数据的预测值state。input 和真实值 state。truth求出 经过网络后数据的变化量delta 以及错误output
        smooth_l1_cpu(l.batch*l.inputs, state.input, state.truth, l.delta, l.output);
    } else {
        l2_cpu(l.batch*l.inputs, state.input, state.truth, l.delta, l.output);//l.output就是error
    }
    //所以网络的cost的输出其实就是将所有的点的错误累加起来
    l.cost[0] = sum_array(l.output, l.batch*l.inputs);
}

void backward_cost_layer(const cost_layer l, network_state state)
{
    //对l.delta进行l.scale的尺度变换得到结果state.delta
